...
app.post("/auth", (req, res) => {
  ...
  var info = contacts.find(user => user.name === name && user.password === password)
  if (info) {
    console.log("Matched");
  }
  else {
  ...
  if (info && info.role == "admin") {
    res.send(`<form action="/contacts" method="get">
  ...
  if (info && info.role != "admin") {
    ...
    <label for="name">Not an Admin: </label>
    <input type="submit" value="Return">
    </form>`);

    console.log("Matched but Not an Admin");  
  }
